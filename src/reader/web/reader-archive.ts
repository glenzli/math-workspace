...
const copy = {
    zh: {
        ...
        compare: '比较版本', compareWith: '选择另一版本', noChanges: '文件内容相同。', raw: '源码', rendered: '查看原稿',
    ...
    en: {
        ...
        compare: 'Compare versions', compareWith: 'Choose another version', noChanges: 'File contents are identical.', raw: 'Source', rendered: 'View original',
...
export class ReaderArchive {
    ...
    close(): void { this.generation++; this.dialog?.close(); this.dialog?.remove(); this.dialog = undefined; this.content = undefined; }
    async open(): Promise<void> {
        ...
        header.append(element('h1', w.title), button(w.close, () => this.close()));
    ...
    private render(): void {
        ...
        tools.append(button(w.verify, () => void this.startJob({ action: 'verify' })), button(w.refresh, () => void this.refresh()),
            button(w.export, () => void this.download()));
        ...
        filter.append(kind, current, trace, button(w.search, () => void this.trace(trace.value)));
        ...
        if (selected) { this.selected = selected.id; this.renderDetail(detail, selected); }
    ...
    private renderDetail(target: HTMLElement, row: ArchiveRow): void {
        ...
        compare.append(element('option', w.compareWith));
        ...
        target.append(compare, button(w.compare, () => void this.compare(row, compare.value, target)));
        const list = element('div', '', 'archive-files');
        for (const name of Object.keys(row.files).filter(name => name.endsWith('.md')).sort()) list.append(button(name, () => void this.read(row.id, name, target)));
        target.append(list);
    }
    private async read(id: string, name: string, target: HTMLElement): Promise<void> {
        const generation = this.generation, request = ++this.viewRequest; this.report(this.words().loading);
        try {
            const source = await this.request(`/api/archive/source?record=${encodeURIComponent(id)}&file=${encodeURIComponent(name)}`);
            if (generation !== this.generation || request !== this.viewRequest || !target.isConnected) return;
            ...
            target.replaceChildren(button(w.returnList, () => this.render()), element('h2', name), element('p', w.sourceHint, 'archive-hint'),
                button(w.rendered, render), button(w.raw, () => article.replaceChildren(element('pre', source.content))), article);
            render(); this.report(''); target.scrollIntoView({ block: 'start' });
    ...
    private async compare(left: ArchiveRow, rightId: string, target: HTMLElement): Promise<void> {
        if (rightId !== 'current' && !/^[a-f0-9]{64}$/.test(rightId)) return;
        const generation = this.generation, request = ++this.viewRequest;
        this.report(this.words().loading);
        try {
            ...
            target.replaceChildren(button(w.returnList, () => this.render()), element('h2', w.compare), element('p', `${leftLabel} → ${rightLabel}`));
            if (!result.changes.length) target.append(element('p', w.noChanges));
            const changes = element('div', '', 'archive-files'), preview = element('div', '', 'archive-diff');
            for (const change of result.changes) changes.append(button(`${w[change.change as 'added' | 'removed' | 'modified']} · ${change.path}`, () => {
                ...
                void Promise.all([readSide(left.id), readSide(rightId)]).then(([a, b]) => {
                    if (generation !== this.generation || previewRequest !== this.viewRequest || !preview.isConnected) return;
                    const before = element('section'), after = element('section');
                    before.append(element('h3', leftLabel), element('pre', a)); after.append(element('h3', rightLabel), element('pre', b));
                    preview.replaceChildren(before, after); preview.scrollIntoView({ block: 'start' });
                }).catch(error => this.report(String(error)));
            })); target.append(changes, preview); this.report(''); target.scrollIntoView({ block: 'start' });
        } catch (error) { this.report(String(error)); }
    }
    private async trace(value: string): Promise<void> {
        ...
        try {
            ...
            this.content.replaceChildren(button(w.returnList, () => this.render()), element('h2', w.formalId));
            if (!result.matches.length) list.append(element('p', w.noMatch));
            for (const match of result.matches) list.append(button(`${this.date(match.loggedAt)} · ${match.filePath}:${match.line}`, () => void this.read(match.recordId, match.filePath, this.content!)));
